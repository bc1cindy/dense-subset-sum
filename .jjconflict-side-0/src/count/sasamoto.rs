...
/// log W(E); log space because W reaches 2^N and overflows f64 past N ≈ 1024.
/// Panics if `e_target` ∉ (0, Σaⱼ).
pub(crate) fn log_w_for_e(a: &[f64], e_target: f64) -> f64 {
...
/// # Panics
///
/// Panics if `a` is empty or all-zero (gcd undefined), or if (after gcd
/// normalization) `e_target` is not in `(0, Σaⱼ)` since no subset can sum there.
#[must_use]
...
/// summing to `e_target` (eq 5.8). Saddle is 2D in (β, μ), Gaussian width is
/// √det(D) from eq 5.9.
///
/// # Panics
///
...
pub(crate) fn log_w_for_m_e(a: &[f64], m_target: usize, e_target: f64) -> f64 {
...
/// # Panics
///
/// Panics if `a` is empty or all-zero, or if `(m_target, e_target/gcd)` is
/// outside the feasibility interior (sums of the `m_target` smallest/largest
/// elements of `a`); equal-element A collapses the interior.
#[must_use]
...
/// Sasamoto saddle width `⌊√N / 2⌋` (appendix A.7); the natural M-bound at
/// which the constrained asymptotic is sharp.
#[must_use]
...
#[cfg(test)]
pub(crate) fn n_c_for_m_e(a: &[u64], m: usize, e: u64) -> Option<f64> {
    if m == 0 || m >= a.len() {
        return None;
    }
    let g = gcd_slice(a)?;
...
fn feasibility_bounds(a: &[f64], m_target: usize) -> (f64, f64) {
    assert!(
        m_target > 0 && m_target < a.len(),
        "m_target = {} must be in (0, {})",
        m_target,
        a.len()
    );
...
fn feasibility_assert(a: &[f64], m_target: usize, e_target: f64) {
    let (e_min, e_max) = feasibility_bounds(a, m_target);
    assert!(
        e_min < e_target && e_target < e_max,
        "e_target = {e_target} must be in ({e_min}, {e_max}) for m = {m_target}"
    );
}
...
/// N ∈ [5, 100] across magnitudes up to 2·10¹⁵ (`CoinJoin` satoshi scale).
/// Panics if E ∉ (0, Σaⱼ) or bracket misses the saddle.
fn find_beta(a: &[f64], e_target: f64, mu: f64) -> f64 {
    let e_max: f64 = a.iter().sum();
    assert!(
        0.0 < e_target && e_target < e_max,
        "find_beta: e_target = {e_target} must be in (0, {e_max})"
    );
    ...
    // Convergence criterion is on the residual, not the bracket: at satoshi
    // scale a_max ≈ 10¹⁵ so |Δβ| ≈ 10⁻¹⁵ is needed to resolve a unit in
    // ⟨E⟩, far past `hi - lo < 1e-12`.
    let tol = e_target.abs().max(1.0) * 1e-12;
    for _ in 0..300 {
...
mod tests {
    ...
    fn m_sqrt_over_2_matches_paper_default() {
        assert_eq!(m_sqrt_over_2(16), 2);
        assert_eq!(m_sqrt_over_2(100), 5);
    ...
    fn test_brute_force() {
        ...
        let candidates: Vec<(u64, u64)> = w_exact
            .iter()
            .filter(|&(_, &w)| w >= W_MIN_FOR_ASYMPTOTIC_MATCH)
            .filter(|&(&e, _)| {
                let e_f = e as f64;
                e_f > 0.0 && e_f < e_max
            })
    ...
    fn test_convergence_with_n() {
        fn peak_error(n: usize) -> f64 {
            ...
            let w_exact = (0..(1u64 << n))
                .filter(|&mask| {
                    let sum: f64 = (0..n).filter(|&j| mask & (1 << j) != 0).map(|j| a[j]).sum();
                    (sum - e_mid.round()).abs() < 0.5
                })
                .count();
        ...
        assert!(
            err_18 < err_16,
            "err should decrease: 16={err_16:.4} 18={err_18:.4}"
        );
        assert!(
            err_20 < err_18,
            "err should decrease: 18={err_18:.4} 20={err_20:.4}"
        );
    ...
    fn test_find_beta_converges_across_range() {
        for &n in &[5usize, 10, 20, 50, 100] {
            ...
            let e_max: f64 = a.iter().sum();
            for i in 1..20 {
                let e = e_max * f64::from(i) / 20.0;
                let lw = log_w_for_e(&a, e);
                assert!(lw.is_finite(), "N={n}, E={e}: log W = {lw}");
            }
        }
    }
    ...
    fn test_constrained_brute_force() {
        ...
        let candidates: Vec<(usize, u64, u64)> = w_exact
            .iter()
            .filter(|&(&(m, _), &w)| w >= W_MIN_FOR_ASYMPTOTIC_MATCH && m != 0 && m != n)
            .filter(|&(&(m, e), _)| {
        ...
        for &(m, e, w) in &candidates {
            let lw = log_w_for_m_e(&a, m, e as f64);
            let err = (lw.exp() - w as f64).abs() / w as f64;
            assert!(
                err < 0.25,
    ...
    fn test_marginal_identity() {
        let a: Vec<f64> = (1..=15).map(f64::from).collect();
        let n = a.len();
        ...
        let sum_w_m_e: f64 = (1..n)
            .filter(|&m| {
                let e_min: f64 = sorted.iter().take(m).sum();
                let e_max: f64 = sorted.iter().rev().take(m).sum();
                e_f > e_min && e_f < e_max
            })
            .map(|m| log_w_for_m_e(&a, m, e_f).exp())
            .sum();
    ...
    proptest! {
        ...
        /// Same round-trip as `find_beta_inverts_mean_energy`, but at the
        /// magnitude of an unnormalized CoinJoin instance: aⱼ ∈ [10⁶, 2·10¹⁵]
        /// (1 BTC = 10⁸ satoshis; MAX_MONEY ≈ 2·10¹⁵). Bracket [-200, 200] on
        /// β must still trap the saddle and the f64 round-trip must hold to
        /// the looser tolerance imposed by ULPs at this scale (relative
        /// precision ≈ 10⁻¹⁰ on Σaⱼ ≈ 10¹⁷).
        #[test]
        fn find_beta_inverts_mean_energy_at_satoshi_scale(
            (a, e) in (5usize..30).prop_flat_map(|n| {
                (
                    proptest::collection::vec(1.0e6_f64..2.0e15, n),
                    0.05f64..0.95,
                )
            }).prop_map(|(a, e_frac)| {
                let e = e_frac * a.iter().sum::<f64>();
            ...
            prop_assert!(
                (recovered - e).abs() / e < 1e-6,
                "satoshi-scale round-trip failed: e={}, recovered={}, β={}",
                e, recovered, beta
            );
        }
    ...
    fn test_u64_vs_brute_force() {
        ...
        let candidates: Vec<(u64, u64)> = w_exact
            .iter()
            .filter(|&(&e, &w)| w >= W_MIN_FOR_ASYMPTOTIC_MATCH && e != 0 && e < e_max)
            .map(|(&e, &w)| (e, w))
            .collect();
    ...
    fn test_log_w_for_e_sat_panics_on_empty() {
        let _ = log_w_for_e_sat(&[], 10);
    }

    #[test]
    #[should_panic(expected = "nonzero")]
    fn test_log_w_for_e_sat_panics_on_all_zeros() {
        let _ = log_w_for_e_sat(&[0, 0, 0], 0);
    }
    ...
    fn test_log_w_for_m_e_sat_panics_on_empty() {
        let _ = log_w_for_m_e_sat(&[], 0, 10);
    }

    #[test]
    #[should_panic(expected = "nonzero")]
    fn test_log_w_for_m_e_sat_panics_on_all_zeros() {
        let _ = log_w_for_m_e_sat(&[0, 0, 0], 1, 0);
    }
    ...
    fn test_n_c_for_m_e_conservative_near_boundary() {
        ...
        assert!(
            ...
            "near={nc_near:?}, central={nc_central}"
        );
    }
