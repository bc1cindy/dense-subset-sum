...
use crate::count::companion::log_w_signed;
use crate::count::sparse_conv::Goldilocks;
use crate::count::sumset::GradedSumsetBudget;
use crate::harness::stats::{median, pearson_correlation, spearman_correlation};
use crate::harness::vs_cja::{mappings, signed_to_opt};
...
pub fn validate_estimators(
    ...
    for (i, a) in test_sets.iter().enumerate() {
        ...
        let report = crate::harness::vs_oracle::compare(a, min_w, lookup_k, dp_max_table, &label);
...
pub struct MappingComparison {
    ...
    /// `log₂(n_non_derived)`: Maurer/Boltzmann entropy in bits.
    pub entropy: f64,
    pub sub_tx_estimates: Vec<SubTxEstimate>,
    pub max_log_sasamoto_approx: f64,
...
pub fn compare_w_vs_mappings(
    ...
    compare_w_vs_mappings_with(tx, label, knee, max_coins, FeeHandling::PhantomOutput)
}
...
pub fn compare_w_vs_mappings_with(
    ...
    let balanced_tx = if fee > 0 {
        ...
        outputs.push(u64::try_from(fee).ok()?);
    ...
    let entropy = mappings::boltzmann_entropy(non_derived.len());

    let estimates = estimate_sub_txs(&balanced_tx, knee);

    let max_log_w_sas = estimates
        .iter()
        .filter_map(|e| e.log_sasamoto_approx)
    ...
    let log_w_signed_val = if matches!(fee_handling, FeeHandling::SignedMultiset) {
        signed_to_opt(log_w_signed::<Goldilocks>(
            &tx.inputs,
            &tx.outputs,
            fee,
            knee,
    ...
    Some(MappingComparison {
        ...
        max_log_sasamoto_approx: max_log_w_sas,
        max_log_w_lookup: max_log_w_lkp,
        max_log_w_combined,
        log_w_signed: log_w_signed_val,
    })
}
...
pub fn print_mapping_comparison(mc: &MappingComparison) {
    ...
    println!(
        "  CJA mappings: {} total, {} non-derived (H = {:.2} bits)",
        mc.n_mappings, mc.n_non_derived, mc.entropy
    );
    println!(
        "  max log_w: sasamoto={:.2}, lookup_in={:.2}, combined(in+out)={:.2}",
        mc.max_log_sasamoto_approx, mc.max_log_w_lookup, mc.max_log_w_combined
    );
...
pub fn correlate_w_vs_mappings(comparisons: &[MappingComparison]) -> Option<MappingCorrelation> {
    ...
    for mc in comparisons {
        ...
        let sas = if mc.max_log_sasamoto_approx.is_finite() {
            mc.max_log_sasamoto_approx
        } else {
            0.0
        };
        max_sas.push(sas);
        ...
        let rows: Vec<MappingComparison> = (1..=8)
            .map(|i| {
                ...
                MappingComparison {
                    ...
                    entropy: mappings::boltzmann_entropy(i as usize),
                    sub_tx_estimates: Vec::new(),
                    max_log_sasamoto_approx: log_w - 0.1,
    ...
    fn test_correlation_filters_zero_non_derived() {
        let rows: Vec<MappingComparison> = vec![
            MappingComparison {
                ...
                n_mappings: 1,
                n_non_derived: 0,
                entropy: mappings::boltzmann_entropy(0),
                sub_tx_estimates: Vec::new(),
                max_log_sasamoto_approx: 5.0,
            ...
            MappingComparison {
                ...
                n_mappings: 2,
                n_non_derived: 1,
                entropy: mappings::boltzmann_entropy(1),
                sub_tx_estimates: Vec::new(),
                max_log_sasamoto_approx: 2.0,
            ...
            MappingComparison {
                ...
                n_mappings: 4,
                n_non_derived: 3,
                entropy: mappings::boltzmann_entropy(3),
                sub_tx_estimates: Vec::new(),
                max_log_sasamoto_approx: 3.0,
            ...
            MappingComparison {
                ...
                n_mappings: 6,
                n_non_derived: 5,
                entropy: mappings::boltzmann_entropy(5),
                sub_tx_estimates: Vec::new(),
                max_log_sasamoto_approx: 4.0,
