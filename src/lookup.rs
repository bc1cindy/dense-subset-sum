...
/// Subset-sum DP. `None` if table > `max_cells`; `Some(0)` for unreachable E.
pub fn dp_w(original_set: &[u64], e_target: u64, max_cells: usize) -> Option<u128> {
    let NormalizedDp {
        ...
        DpInput::Empty => panic!("dp_w: A must contain at least one nonzero value"),
        DpInput::SumOverflow => panic!("dp_w: Σa overflows u64"),
    };
...
enum DpInput {
    ...
    EarlyZero,
    /// A is empty or all-zero (gcd undefined).
    Empty,
    /// Σa overflows u64.
    SumOverflow,
}
...
pub fn dp_w_restricted(
    ...
    let NormalizedDp {
        ...
        DpInput::Ready(input) => input,
        DpInput::EarlyZero => return Some(0),
        DpInput::Empty => panic!("dp_w_restricted: A must contain at least one nonzero value"),
        DpInput::SumOverflow => panic!("dp_w_restricted: Σa overflows u64"),
    };
...
mod tests {
    ...
    #[test]
    #[should_panic(expected = "must contain at least one nonzero value")]
    fn test_dp_w_empty_input_panics() {
        dp_w(&[], 0, 1_000_000);
    }

    #[test]
    #[should_panic(expected = "must contain at least one nonzero value")]
    fn test_dp_w_all_zero_panics() {
        dp_w(&[0, 0, 0], 0, 1_000_000);
    }

    #[test]
    #[should_panic(expected = "Σa overflows u64")]
    fn test_dp_w_sum_overflow_panics() {
        let a: Vec<u64> = vec![u64::MAX, 1];
        dp_w(&a, 0, 1_000_000_000);
    }
    ...
    #[test]
    #[should_panic(expected = "Σa overflows u64")]
    fn test_dp_w_restricted_sum_overflow_panics() {
        let a: Vec<u64> = vec![u64::MAX, 1];
        dp_w_restricted(&a, 1, 0, 1_000_000_000);
    }
