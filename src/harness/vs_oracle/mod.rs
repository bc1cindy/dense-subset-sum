...
mod tests {
    ...
    fn test_wasabi2_cja_mappings() {
        use crate::{fixtures, harness::vs_cja};
        ...
        for (label, tx) in fixtures::all_wasabi2_false_cjtxs() {
            match vs_cja::compare_w_vs_mappings(&tx, label, lookup_k, max_coins) {
                Some(mc) => {
                    vs_cja::print_mapping_comparison(&mc);
                    comparisons.push(mc);
                }
                None => {
        ...
        if !comparisons.is_empty() {
            vs_cja::print_mapping_summary(&comparisons);
        }
