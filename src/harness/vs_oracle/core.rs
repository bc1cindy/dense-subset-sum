...
pub struct ComparisonRow {
    ...
    pub w_exact: f64,
    pub sasamoto_approx: Option<f64>,
    pub w_lookup: Option<u128>,
    pub dp_w: Option<u128>,
...
pub fn compare_dp_ground_truth(
    ...
    let n = a.len();
    let g = gcd_slice(a).ok_or("compare_dp_ground_truth: empty or all-zero input")?;
...
fn kappa_of(a: &[u64]) -> Option<f64> {
    let l = a.iter().copied().max()?;
    kappa(l, a.len())
}

fn sasamoto_w(a: &[u64], e: u64) -> Option<f64> {
    let lw = log_w_for_e_sat(a, e);
    if lw.is_finite() { Some(lw.exp()) } else { None }
}

fn lookup_w(a: &[u64], e: u64, knee: usize) -> Option<u128> {
    if a.is_empty() || e == 0 {
        return None;
    }
    let s: GradedSumset<Goldilocks> = GradedSumset::bounded(a, &[e], knee.min(a.len()));
    match s.count_total(e) {
        Count::Confirmed(n) | Count::Truncated(n) => Some(u128::from(n)),
        Count::Absent | Count::Unknown => None,
    }
}
