...
/// Collapse [`crate::SignedError`] into the `Option<f64>` form harness consumers expect:
/// `Unreachable` preserved as `NEG_INFINITY`, `EmptyInput` mapped to `None`.
pub(super) fn signed_to_opt(r: Result<f64, crate::SignedError>) -> Option<f64> {
    use crate::SignedError;
    match r {
        Ok(v) => Some(v),
        Err(SignedError::Unreachable) => Some(f64::NEG_INFINITY),
        Err(SignedError::EmptyInput) => None,
    }
}
...
mod tests {
    ...
    fn test_compare_estimators_n20() {
        ...
        let report = crate::harness::vs_oracle::compare(&a, 100, 10, 1_000_000, "n20");
    ...
    fn test_compare_gcd_values() {
        ...
        let report = crate::harness::vs_oracle::compare(&a, 10, 8, 10_000_000, "gcd");
