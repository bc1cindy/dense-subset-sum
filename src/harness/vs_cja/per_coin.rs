...
pub struct CoinMeasurement {
    ...
    /// κ_c evaluated at this coin's natural target. `None` when N · L · value
    /// doesn't yield a feasible normalized position in (0, 1).
    pub kappa_c_at_value: Option<f64>,
}

/// Zero-fee signed probe: count signed partitions of the other coins balancing `value`.
pub fn per_coin_measurements(tx: &Transaction, knee: usize) -> Vec<CoinMeasurement> {
    per_coin_measurements_inner(tx, knee, false)
}

/// Input target = v − round(F · v / Σin); outputs unchanged (they don't pay fees).
pub fn per_coin_measurements_fee_aware(tx: &Transaction, knee: usize) -> Vec<CoinMeasurement> {
    per_coin_measurements_inner(tx, knee, true)
}
...
pub fn print_per_coin_measurements(
    ...
    println!(
        "{:>4} {:>4} {:>15} {:>15} {:>10}",
        "role", "idx", "value", "log2_log_w_signed", "κ_c"
    );
    println!("{:─<52}", "");
    ...
    println!(
        "  coins with log2_log_w_signed ≥ 5: {}/{} total  (in {}/{}, out {}/{})",
        large_in + large_out,
        measurements.len(),
        large_in,
        n_in,
...
fn per_coin_measurements_inner(
    ...
    let l_in = tx.inputs.iter().copied().max().unwrap_or(0);
    let kappa_c_at = |value: u64| -> Option<f64> {
        if n_in == 0 || l_in == 0 {
            return None;
        }
        kappa_c(value, n_in, l_in)
    };

    let budget = GradedSumsetBudget::<Goldilocks>::default();
    ...
    for (i, &value) in tx.inputs.iter().enumerate() {
        ...
        let lw = signed_to_opt(log_w_signed(
            &other_outputs,
            &other_inputs,
            target,
            knee,
...
mod tests {
    ...
    fn test_per_coin_measurements_wasabi2_small() {
        ...
        assert!(
            any_nontrivial,
            "expected at least one coin with log2_log_w_signed >= 1"
        );
    }
    ...
    fn test_per_coin_measurements_populate_kappa_c_at_value() {
        ...
        let measurements = per_coin_measurements(&tx, 3);
        let n = tx.inputs.len();
        let l = *tx.inputs.iter().max().unwrap();
        for m in &measurements {
            let expected = crate::kappa_c(m.value, n, l);
    ...
    fn test_per_coin_measurements_large_n_works() {
        ...
        assert!(
            reachable >= 100,
            "at N=160, measurements should reach for most coins, got {}/160",
            reachable
        );
    }
