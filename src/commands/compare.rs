...
pub(crate) fn cmd_compare_empirical(
    n: usize,
    seed: u64,
    min_w: u64,
    block_size: usize,
    ...
    if !path.exists() {
        eprintln!(
            "error: {} not found. Run scripts/fetch_cja_distribution.sh to download the CJA distribution (~353 MB).",
            path.display()
        );
        std::process::exit(1);
    }
    let dist = comparison::EmpiricalDistribution::from_cja_bin(path).unwrap_or_else(|e| {
        eprintln!(
            "error: failed to parse CJA distribution at {}: {}\nThe file may be corrupted; re-run scripts/fetch_cja_distribution.sh.",
            path.display(),
            e
        );
        std::process::exit(1);
    ...
    println!(
        "Using CJA distribution.bin ({} CDF points) from {}",
        dist.cdf_len(),
        path.display()
    );
    let raw = dist.random_set(n, seed);
    let values: Vec<u64> = if divisor > 1 {
        raw.iter().map(|v| v / divisor).filter(|&v| v > 0).collect()
    } else {
        raw.clone()
    };
    if divisor > 1 {
        println!(
            "Rescaled by /{}: kept {}/{} non-zero values; sum {} → {}",
            divisor,
            values.len(),
            raw.len(),
            raw.iter().sum::<u64>(),
    ...
    if values.is_empty() {
        eprintln!(
            "error: all values rounded to zero under divisor={}",
            divisor
        );
        std::process::exit(1);
    }
