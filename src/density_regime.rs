...
pub struct Bracket {
    pub kappa: Interval<f64>,
    pub kappa_c: Interval<f64>,
}
...
impl Bracket {
        ...
        I::IntoIter: ExactSizeIterator,
    {
        ...
        Self {
            kappa: Interval {
                best: (l_best as f64).log2() / n as f64,
                worst: worst_case_kappa(n),
            },
            kappa_c: Interval {
                best: kappa_c(e_target, n, l_best),
                worst: kappa_c(e_target, n, l_worst),
            },
        }
    }
    ...
    pub fn regime(&self) -> Regime {
        if self.kappa.worst < self.kappa_c.worst {
            Regime::Dense
        } else if self.kappa.best >= self.kappa_c.best {
            Regime::Sparse
        } else {
            Regime::Transitional
        }
...
fn trapezoidal_integral<F: Fn(f64) -> f64>(f: F, a: f64, b: f64, n: usize) -> f64 {
    let h = (b - a) / n as f64;
    let interior: f64 = (1..n).map(|i| f(a + i as f64 * h)).sum();
    ((f(a) + f(b)) / 2.0 + interior) * h
}
...
mod tests {
    ...
    fn test_best_case_kappa_paper_example() {
        ...
        let k = Bracket::new(a.iter().copied(), e).kappa.best;
    ...
    fn test_best_case_kappa_decreases_with_n() {
        ...
        let k10 = Bracket::new(a10.iter().copied(), e10).kappa.best;
        let k20 = Bracket::new(a20.iter().copied(), e20).kappa.best;
    ...
    fn test_density_regime_new_empty_panics() {
        Bracket::new(std::iter::empty::<u64>(), 1);
    }
    ...
    fn test_kappa_c_at_half_is_zero() {
        ...
        let dr = Bracket::new(a.iter().copied(), e_mid);
    ...
    fn test_kappa_c_equal_values_whirlpool() {
        ...
        let dr = Bracket::new(a.iter().copied(), e_total);
        assert_eq!(dr.kappa_c.best, 0.0);
        assert!(
            matches!(dr.regime(), Regime::Sparse),
    ...
    /// Dense requires the pessimistic L (MAX_MONEY/N) to still satisfy κ < κ_c.
    /// Construct it with N=100 equal coins and E=MAX_MONEY/4 so x_worst lands at
    /// the κ_c peak (≈ 1.0) while κ_worst ≈ 0.44.
    #[test]
    fn test_regime_dense_high_n() {
        let n = 100usize;
        let e_target = MAX_MONEY / 4;
        let coin = e_target / n as u64;
        let a: Vec<u64> = vec![coin; n];
        let dr = Bracket::new(a.iter().copied(), e_target);
        assert!(
